...
module "lambda_function_tts" {
  ...
  environment_variables = {
    OPENAI_API_KEY = jsondecode(data.aws_secretsmanager_secret_version.secret_credentials.secret_string)["OPENAI_API_KEY"]
...
# Access openai API Key from secrets manager
data "aws_secretsmanager_secret" "openai_api_key" {
  name = var.secrets_manager_openai_api_secret_name
}
